...
> `$maintain-implementation-notes`; place evidence under `<evidence>`. Determine the ticket's blockers and acceptance observations before editing.
> Maintain an active-assignment ledger at `<evidence>/plan.md` with agent, role, question or deliverable, owned files or seams, dependencies, message
> recipient, and status. Check it before every spawn. Own this ticket end to end: scout, plan thin slices, delegate bounded non-overlapping leaves,
...
> ticket; changed files and commit hashes; criteria met or deferred with reasons; tests with underlying exit codes and executed counts; snapshot
...
Add the exact authorization boundaries, repository-specific restrictions, configured concurrency capacity, and canonical collaborator targets to this
assignment. Do not rely on inherited conversation context. Adapt leaf concurrency to the available capacity without creating overlapping work.
...
> dirty state. Do not edit the shared implementation-notes page, ticket status, worklog, or evidence owned by another agent. Do not commit. Run only
...
> Make no edits, commits, status changes, or device actions. Complete this assignment directly. Do not spawn other agents. Your parent's delegation
...
> code, commits, ticket status, worklog, or the shared notes page. Return the evidence table, material peer messages, and proposed notes entries.
...
> Audit `<ticket>` using `<evidence>`, `<notes>`, `<evidence>/plan.md`, the ticket coordinator's report, the actual diff or commits, and current
...
> state. Message every gap to the live ticket coordinator `<ticket-coordinator-target>`, then summarize those messages in your report. List every
...
> peer-message dependencies, evidence, `git status`, and authorized commits. Preserve valid landed work, verify it before relying on it, and continue
> from the first unmet criterion. Do not trust the prior coordinator's completion label.
