...
description: >-
  ...
  simulated-device, Device Hub, Playbook snapshot, or runtime QA work that needs exact simulator ownership, isolated DerivedData and evidence,
  raw logs, record-inspect-compare, and explicit lane handoff.
---
...
For Device Hub QA, verify Device Hub Info shows the leased UUID before launch and after every Xcode destination-picker change.
Generic labels such as `iPhone 17 Pro` are not proof. Read the accessibility tree before screenshots, activate the exact control on the exact
surface, and keep Playbook and production callback claims separate.
